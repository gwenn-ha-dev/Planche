...
func drawGlyph(_ ctx: CGContext, _ box: CGRect) {
    // A contact sheet: nine frames on a grid.
    let w = box.width, gap = w * 0.085, cell = (w - 2 * gap) / 3
    ctx.setFillColor(white(1))
    for row in 0..<3 {
        for col in 0..<3 {
            let r = CGRect(x: box.minX + Double(col) * (cell + gap),
                           y: box.minY + Double(row) * (cell + gap),
                           width: cell, height: cell)
            ctx.addPath(squircle(r, cell * 0.22))
            ctx.setFillColor(white(row == 1 && col == 1 ? 1 : 0.82))
            ctx.fillPath()
        }
    }
}
